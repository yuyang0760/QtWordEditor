...
### 20:00:00 - 数学公式编辑器设计方案确定

**问题描述**：
- 讨论数学公式编辑器的实现方案
- 对比了多种方案：混合绘制、完全直接绘制、混合子项
- 讨论了TextFragment的实现方式（继承QGraphicsItem vs QGraphicsTextItem）

**方案讨论历史**：

1. **初始方案**（2026-02-21文档）：混合绘制
   - 文本：QPainter直接绘制
   - 公式：MathItem（QGraphicsItem子项）
   - 问题：文本和公式处理方式不一致

2. **方案二讨论**：完全直接绘制
   - 所有内容（文本+公式）都用QPainter直接绘制
   - 问题：事件处理极其复杂，需要手动计算点击位置

3. **TextFragment实现讨论**：
   - 方案A：继承QGraphicsTextItem
     - 优点：开发快，功能全
     - 缺点：黑盒，基线对齐难
   - 方案B：继承QGraphicsItem，自己绘制
     - 优点：完全可控，基线对齐精确
     - 缺点：需要自己实现文本绘制
   - **最终选择：方案B**
...
**整合文档**：
- 创建了新的设计文档：`u文档/数学公式编辑器完整设计文档_最终方案.md`
- 整合了两个原有文档的优点
- 确定了最终的架构方案
