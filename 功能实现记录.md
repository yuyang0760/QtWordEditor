...
**问题描述**：
- 原设计文档中存在两种光标方案：双光标方案（文本光标 + 公式光标）和统一光标方案
- 用户希望统一使用一个光标方案，避免混淆
- 需要将设计文档中的光标方案最终统一为统一光标方案

**修改内容**：

1. **更新数学公式编辑器设计文档** (`d:\vscodeproject\QtWordEditor\u文档\数学公式编辑器完整设计文档_最终方案.md`)
   - 更新文档标题和版本号（v5.0.0）
   - 更新整体架构图，用统一光标替代双光标
   - 统一光标方案移到主要位置（4.6节），双光标方案标记为备选（4.7节）
   - 更新TextBlockItem类设计，移除MathCursor相关方法，添加统一光标支持方法
   - 更新完整架构总结：视图层、交互流程、方案优势总结
   - 更新实现步骤：将统一光标实现作为阶段六
   - 更新方案决策历史记录，添加统一光标决策

2. **统一光标方案核心改进**：
   - `CursorPosition` 扩展：添加 `CursorLocationType` 枚举（Text/Formula）
   - `CursorLocationType`：区分文本位置和公式位置
   - `FormulaCursorInfo`：包含公式内部的位置信息
   - `Cursor` 类扩展：重写导航方法，支持进入/退出公式
   - 新增信号：`enteredFormula()`、`exitedFormula()`
   - 统一光标：一个Cursor同时处理文本和公式，无缝切换

3. **TextBlockItem类更新**：
   - 移除MathCursor前向声明，添加RowContainerItem
   - 移除公式编辑模式相关方法（isInMathEditMode、enterMathEditMode等）
   - 添加统一光标支持方法：
     - `getMathSpanAtOffset()`：获取指定偏移处的公式
     - `getOffsetForMathSpan()`：获取公式的偏移量
     - `getMathSpanRect()`：获取公式的矩形区域
     - `getFormulaVisualPosition()`：获取公式内部视觉位置
   - 更新成员变量：
     - 移除m_inMathEditMode、m_rootMathItem、m_mathCursor
     - 添加m_mathItemMap（MathSpan到视图映射）
     - 添加m_offsetToMathSpanMap（文本偏移到MathSpan映射）

**文档更新总结**：
- 文档标题：统一光标方案 + 混合子项方案
- 文档版本：v5.0.0
- 主要方案：统一光标方案（一个Cursor处理文本和公式）
- 备选方案：双光标方案（MathCursor）
- 完整架构总结更新：视图层、交互流程、方案优势
- 实现步骤：阶段六为统一光标实现

**预期收益**：
- ✅ 光标方案统一，避免混淆
- ✅ 一个Cursor同时处理文本和公式，无缝切换
- ✅ 无模式切换，用户感觉不到文本和公式的区别
- ✅ 状态一致，不会出现两个光标同时显示的问题
- ✅ 设计文档结构清晰，主要方案和备选方案明确区分
...
**最终方案架构**：
```
TextBlockItem (QGraphicsItem)
├── TextFragment (QGraphicsItem，自己绘制文本)
├── MathFormulaItem (QGraphicsItem，绘制公式)
